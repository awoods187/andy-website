...
I tried everything. Two Claude Code windows to separate planning from execution scattered context between sessions and hit window limits. Opus for design had better reasoning and easier context control but couldn't see the codebase. Each approach solved one problem while creating another.

Then I discovered three phases that worked. Design in Opus for deep reasoning. Validate against the codebase in Claude Code to catch integration issues. Implement in a fresh Claude Code session for clean execution.
...
The [Architect-Builder Pattern](https://github.com/awoods187/PM-Prompt-Patterns/tree/main/prompts/product-strategy/architect-builder-pattern) is emerging across our community as a repeatable workflow that actually works.

## The Architect-Builder Pattern

The pattern separates AI development into three distinct phases, each with a specific model and purpose:

**Phase 1: The Architect** uses Claude Opus for high-level design and architectural decisions. This phase focuses on the "why" and "what" of your solution, producing a detailed architecture blueprint without touching code. Opus excels here because it handles ambiguity and complex reasoning better than any other model.
...
**Phase 2: The Validator** takes the design into Claude Code with full codebase context. This phase catches integration issues before any code is written—finding naming conflicts, identifying existing patterns to follow, and spotting architectural mismatches. It transforms the abstract design into a concrete implementation plan.

**Phase 3: The Builder** starts fresh in a new Claude Code session with only the validated implementation plan. No context pollution, no conflicting instructions—just clean execution. The fresh session ensures Claude Code follows the plan without getting confused by earlier exploratory conversations.
...
📋 **Full templates and detailed prompts**: [GitHub Repository](https://github.com/awoods187/PM-Prompt-Patterns/tree/main/prompts/product-strategy/architect-builder-pattern)
...
Phase 2 revealed the gaps. Now we convert those findings into explicit constraints.

The Phase 3 prompt directly addresses what validation caught:
...
The pattern's value: converting validation findings into implementation constraints. We open a fresh Claude Code session so it focuses on execution, not analysis.
...
This creates automatic documentation for future development. Phase 1 designs become stakeholder-ready documentation. The validator teaches codebase patterns. Single-focus phases reduce cognitive load. The workflow creates documentation for easy knowledge transfer.
